...
if (file_exists(INC_PATH . 'config/install.lock.php'))
{
    H::redirect_msg(loadClass('core_lang')->_t('您的程序已经安装, 要重新安装请删除 system/config/install.lock.php'));
}

@set_time_limit(0);

View::assign('page_title', 'WeCenter - Install');
View::assign('static_url', '../static');
...
switch ($_POST['step'])
{
    default :
        ...
        if (version_compare(PHP_VERSION, LOWEST_PHP_VERSION, '>=') AND get_cfg_var('safe_mode') == false)
        {
        ...
        // 检测 INC_PATH 是否有写权限
        if (is_really_writable(INC_PATH) OR defined('IN_SAE'))
        {
            $system_require['config_writable_core'] = TRUE;
        }

        // 检测 INC_PATH /config/ 是否有写权限
        if (is_really_writable(INC_PATH . 'config/') OR defined('IN_SAE'))
        {
        ...
        if (!defined('IN_SAE'))
        {
            ...
            {
                $error_messages[] = loadClass('core_lang')->_t('你的主机无法与微博通讯, 相关功能将不能使用');
            }

            if (!@gethostbyname('graph.qq.com'))
            {
                $error_messages[] = loadClass('core_lang')->_t('你的主机无法与 QQ 通讯, QQ 登录功能将不能使用');
            }
        }

        View::assign('error_messages', $error_messages);
        View::assign('system_require', $system_require);
        View::assign('base_dir', $base_dir);

        View::output('install/index');
        break;
    ...
    case 2 :
        if (!defined('IN_SAE'))
        {
            ...
            foreach ($data_dir as $key => $dir_name)
            {
                if (! is_dir(ROOT_PATH . $dir_name))
                {
                    ...
                    {
                        $error_messages[] = loadClass('core_lang')->_t('目录: %s 无法创建，请将网站根目录权限设置为 777, 或者创建这个目录设置权限为 777', ROOT_PATH . $dir_name);
                    }
                }
            }

            if (! is_really_writable(INC_PATH))
            {
                $error_messages[] = loadClass('core_lang')->_t('目录: %s 无法写入，请将此目录权限设置为 777', INC_PATH);
            }
        }
        ...
        if (class_exists('PDO', false))
        {
            if (defined('PDO::MYSQL_ATTR_USE_BUFFERED_QUERY'))
            {
                View::assign('pdo_support', TRUE);
            }
        }

        if (function_exists('mysqli_close'))
        {
            View::assign('mysqi_support', TRUE);
        }

        View::assign('error_messages', $error_messages);
        View::output('install/settings');
        break;
    ...
    case 3 :
        ...
        {
            H::redirect_msg(loadClass('core_lang')->_t('数据库连接失败, 错误信息:') . ' ' . strip_tags($e->getMessage()), './');
        }
        ...
        {
            H::redirect_msg(loadClass('core_lang')->_t('安装中止: WeCenter 要求使用 MySQL 5.0 以上版本的数据库支持, 您的服务器当前 MySQL 版本为: %s', $db->getServerVersion()), './');
        }

        if (!$_POST['db_prefix'] AND count($tables) > 0)
        {
            H::redirect_msg(loadClass('core_lang')->_t('数据库已经存在数据表, 不允许安装, 如要重新安装请先清空数据表'), './');
        }
        ...
        foreach ($tables AS $key => $table_info)
        {
            ...
            foreach ($table_info AS $_key => $table)
            {
                ...
                {
                    H::redirect_msg(loadClass('core_lang')->_t('数据库已经存在相同前缀的数据表, 不允许安装, 如要重新安装请先清空数据表'), './');
        ...
        if (!defined('IN_SAE'))
        {
            ...
            loadClass('core_config')->set('database', $config);
        }
        ...
        View::output('install/final');
        break;

    case 4 :
        $db = loadClass('core_db')->setObject('master');
        $db_prefix = loadClass('core_config')->get('database')->prefix;
        ...
        $db->insert($db_prefix . 'integral_log', array(
             ...
             'action' => 'REGISTER',
             'integral' => 2000,
             'note' => loadClass('core_lang')->_t('初始资本'),
             'balance' => 2000,
             'time' => time()
        ));
        ...
        if (!defined('IN_SAE'))
        {
            $config_file = file_get_contents(INC_PATH . 'config.dist.php');
            ...
            file_put_contents(INC_PATH . 'config.inc.php', $config_file);
            file_put_contents(INC_PATH . 'config/install.lock.php', time());
        }

        View::output('install/success');
        break;
}

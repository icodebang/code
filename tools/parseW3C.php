...
class ParseW3c
{
    ...
    /**
     * 图片url的基准uri。 用来拼接图片url，然后获取图片
     */
    public $imgBaseUri = 'https://www.w3cschool.cn';
    /**
    ...
    protected function processImage ()
    {
        $attachAccessKey = '';
        // 将图片转换成本站内容
        $imgList = $this->domContent->getElementsByTagName('img');
        ...
        for($i=$imgLength-1; $this->doLoadImgFlag && $i>=0; $i--) {
            ...
            while($tryTimes-- > 0) {
                if ( (strpos($_src, 'http://')===0 || strpos($_src, 'https://')===0) && strpos($_src, 'runoob')) {
                    ...
                    $_src = $this->imgBaseUri . $_src;
                } else if (strpos($_src, 'http') !==0) {
                    $_src = $this->imgBaseUri . '/' . $this->currentDirname . '/' . $_src;
                } else { // 已http开头， 保留路径
                    break;
                }
                ...
                //echo __LINE__, "\r\n";
                if ($_imgData) {
                    $attachAccessKey = md5($this->currentFilename.'icodebang.com');
                    try {
    ...
    {
        $content = '';
        if ($this->contentDomInfo['id']) {
            $contentDom = $this->dom->getElementById($this->contentDomInfo['id']);
        } else if ($this->contentDomInfo['tag'] && $this->contentDomInfo['class'] && $this->contentDomInfo['class_index']) {
            $domList = $this->dom->getElementsByTagName($this->contentDomInfo['tag']);
            $index = 0;
            foreach ($domList as $_tmpDom) {
                if (! $_tmpDom->hasAttribute('class')
                 || strpos($_tmpDom->getAttribute('class'),$this->contentDomInfo['class']) ===false ) {
                    continue;
                }
                if ($index == $this->contentDomInfo['class_index']) { // 找到了对应的dom
                    $contentDom = $_tmpDom;
                    break;
                }
                $index++;
    ...
    protected function parseMenuList ()
    {
        // 从左侧菜单解析章节列表
        $chapterList = array();
        ...
        $chapterElements = $menuContainerDom->getElementsByTagName('a');
    ...
    protected function parseError ()
    {
        ...
        if ($this->errorDomInfo['id']) {
         ...
         && $this->errorDomInfo['class_index']) {
            $domList = $this->dom->getElementByTagName($this->errorDomInfo['tag']);
            $index = 0;
            foreach ($domList as $_tmpDom) {
                if (! $_tmpDom->hasAttribute('class')

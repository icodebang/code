    ...
    function  showRandomImg ($elements, imgList, showImgNum, showingQueue) {
        ...
        if (typeof(showingQueue) != 'undefined') {
            ...
            while (true) {
                ...
                $elements.eq(randomNum).html('<img class="wow fadeIn" data-wow-delay="0.6s" style="max-width:100%;visibility: visible; animation-delay: 0.6s; animation-name: fadeIn;" data-length="'+imgList[0]+'" src="' + G_STATIC_URL +imgList[0]+'"/>');
            ...
            while (max > showingQueue.length ) {
                ...
                if (! flagElementHasImg) {
                    $elements.eq(randomNum).html('<img style="max-width:100%;" data-length="'+showingQueue.length+'" src="'+G_STATIC_URL+imgList[showingQueue.length]+'"/>');

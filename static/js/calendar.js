...
function calendar(y, m) {
    ...
    if (m == 2 || m == 3) {
        ...
        if (m == estDay.m && sFtv.length > 10) { // 阳历节日 > 10个， 说明启用了阳历节日
            console.info(sFtv);
    ...
    //黑色星期五
    if ((this.firstWeek + 12) % 7 == 5 && sFtv.length > 10)  // 阳历节日 > 10个， 说明启用了阳历节日
...
function drawCalendar(SY, SM) {
    ...
    if ($('#GD1').parent().find('.unover').length==7) {
        $('#GD1').parent().hide();
    } else {
        $('#GD1').parent().show();
    }

    if ($('#GD41').parent().find('.unover').length==7) {
        $('#GD41').parent().hide();
    } else {
        $('#GD41').parent().show();
    }
...
var dateSelection = {
    ...
    hide : function() {
        ...
        console.info(document.getElementById('dateSelectionDiv'));
    },
    today : function() {
        var today = new Date();
    ...
    changeView : function() {
        ...
        drawCalendar(global.currYear, global.currMonth);
...
/*初始化日期*/
function loadCalendarData(initSelectedDays) {
    if (initSelectedDays) {
        for(var i=0; i<initSelectedDays.length; i++) {
            hDays.push(initSelectedDays[i]);
        }
    }
    initRiliIndex();
    clear();

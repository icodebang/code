...
( function() {
	...
	var bbcodeMap = { b: 'strong', u: 'u', i: 'em', color: 'span', size: 'span', quote: 'blockquote', code: 'pre', url: 'a', email: 'span', img: 'span', '*': 'li', list: 'ol', pre: 'pre', video: 'video', audio: 'audio'},
		convertMap = { strong: 'b', b: 'b', u: 'u', em: 'i', i: 'i', code: 'code', li: '*', pre: 'code'},
		tagnameMap = { strong: 'b', em: 'i', u: 'u', li: '*', ul: 'list', ol: 'list', code: 'code', a: 'link', img: 'img', blockquote: 'quote', attach:'attach' },
		stylesMap = { color: 'color', size: 'font-size' },
		attributesMap = { url: 'href', email: 'mailhref', quote: 'cite', list: 'listType', img:'attach-id' };
		...
		proto: {
			...
			openTagClose : function( tag ) {
				if ( tag == 'br' )
					...
					// icodebang.com 处理附件中， 不写入
					if (this.isProcessingAttach === true) {
						return;
					}
			...
			attribute : function( name, val ) {
				// icodebang.com 处理附件
				if (name=='attach-id' && /[0-9]+/.test(val)) {
					this.isProcessingAttach = true;
					this._.output.pop();
					this.write('[attach]', val);
				}
				// icodebang.com 处理附件中， 不写入
				if (this.isProcessingAttach === true) {
					return;
				...
				elements: {
					$: function( element ) {
						...
						if ( tagName in convertMap )
							...
							if (attributes['attach-id'] ) {
								value = attributes['attach-id'];
							}

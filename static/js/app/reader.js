...
$(document).ready(function () {
	$('#left_panel .icb-mod ul').css('height', ($(window).height() - $('#left_panel .icb-mod').offset()['top']));
	// 侧边导航添加选中样式
...
function view_answer(answer_id) {
	...
	$('.icb-answer-info a').attr('href', G_BASE_URL + '/user/' + answers_list[answer_id]['uid']);
	...
	if (answers_list[answer_id]['agree_users']) {
		$.each(answers_list[answer_id]['agree_users'], function (i, name) {
			...
			{
				$('#vote_info_users_list').append('<li><a href="' + G_BASE_URL + '/user/' + i + '">' + name + '</a>、</li>');
			}
			else
			{
				$('#vote_info_users_list_more').append('<li><a href="' + G_BASE_URL + '/user/' + i + '">' + name + '</a>、</li>');
			}
		});

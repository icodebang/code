$(function()
{
	...
	$('.icb-mod.side-nav a').click(function () {

		if ($('#main_title').attr('id') != null && $(this).attr('rel'))
		{
			$('.icb-mod.side-nav a').removeClass('active');
	...
	$('#bp_more').click(function()
	{
		...
		{
			$('.icb-feed-list').addClass('icb-topic-list');

		}
		else
		{
			$('.icb-feed-list').removeClass('icb-topic-list');
		}
	...
	if ($('.icb-mod.side-nav a[rel="' + window.location.hash.replace(/#/g, '') + '"]').attr('href'))
	{
		$('.icb-mod.side-nav a[rel="' + window.location.hash.replace(/#/g, '') + '"]').click();
	}
	else
	{
		$('.icb-mod.side-nav a[rel=all]').click();
	}
...
function welcome_step(step)
{
	...
	{
		case '1':
			var fileupload = new FileUpload('avatar', $('#welcome_avatar_uploader'), $("#icb-upload-img"), G_BASE_URL + '/account/ajax/avatar_upload/', {'loading_status' : '#icb-img-uploading'});
		break;

		case '2':
			$('#welcome_topics_list').html('<p style="padding: 15px 0" align="center"><img src="' + G_STATIC_URL + '/common/loading_b.gif" alt="" /></p>');

			$('.icb-first-login').hide().siblings().eq(1).show();
		...
		case '3':
			...
			$('.icb-first-login').hide().siblings().eq(2).show();

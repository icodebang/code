$(function () {
    ...
    $('.icb-item-info li .mod-set-head').click(function ()
    {
        ...
        if ($element.is(':visible')) {
            ...
            $(this).closest('li').siblings('li').find('.mod-set-body').slideUp();
        }
    });
    ...
    $(".icb-item-info").find('ul:first').dragsort({
        dragEnd: function () {
        	...
        	setTimeout(function(){ICB.V.dragsortIsRunning = false;}, 200);
    ...
    // input 单选框全选or 全取消
    $('.icb-content-wrap .table').find(".check-all").on('ifChecked', function (e)
    {
        e.preventDefault();
    ...
    $('.icb-content-wrap .table').find(".check-all").on('ifUnchecked', function (e)
    {
        e.preventDefault();
...
function subjectData(type)
{
    ...
    $('#icb-loading').css({top:tempTop+'px',left:tempLeft+'px',position:'absolute'});

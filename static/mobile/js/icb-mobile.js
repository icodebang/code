var AWS = 
{
	...
	loading: function (s) 
	{
		if ($('#icb-loading').length == 0)
	    {
	        $('#icb-modal-window').append('<div id="icb-loading" class="hide"><div id="icb-loading-box"></div></div>');
	    }
	...
	dialog: function (type , data)
	{
		...
		if (template)
		{
			$('#icb-modal-window').empty().append(template);
	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			if (type == 'comments_form')
			{
	        	...
	        	$('#icb-comment-box-' + result.rsm.type_name + '-' + result.rsm.item_id + ' form input').val('');
	        ...
	        else
	        {
	        	switch (type)
	        	{
					...
					case 'reply':
						...
						{
							$('.icb-replay-list ul').append(result.rsm.ajax_html);

							// 文章
							$('.icb-replay-box.article textarea').val('');
							...
							if (USER_ANSWERED)
							{
								$('.icb-replay-box.question').append('<p align="center">一个问题只能回复一次, 你可以在发言后 ' + ANSWER_EDIT_TIME + ' 分钟内编辑回复过的内容</p>');
							}
...
AWS.User = 
{
	...
	invite_user: function(selector, img)
	{
	    ...
	    }, function (result)
	    {
	    	if (result.errno != -1)
	    	{
	    		if (selector.parents('.icb-invite-box').find('.users-list a').length == 0)
	            {
	                selector.parents('.icb-invite-box').find('.users-list').show();
	                selector.parents('.icb-invite-box').find('.users-list').append('<span class="color-999">' + _t('已邀请') + ':</span>');
	            }

	            selector.parents('.icb-invite-box').find('.users-list').append(' <a><img class="img" width="20" src='+ img +' /></a>');

	            selector.parents('.icb-invite-box').find('.icb-dropdown-list').hide();
	...
	disinvite_user: function (selector)
	{
	    $.get(G_BASE_URL + '/question/ajax/cancel_question_invite/question_id-' + QUESTION_ID + "__recipients_uid-" + selector.attr('data-id'), function (result)
		{
			if (result.errno != -1)
	        {
	            $.each($('.icb-question-detail-title .invite-list a'), function (i, e)
	            {
	            ...
	            selector.parents('.icb-question-detail-title').find('.icb-invite-replay .badge').text(parseInt(selector.parents('.icb-question-detail-title').find('.icb-invite-replay .badge').text()) - 1);
	...
	article_vote: function (selector, article_id, rating)
	{
		...
		$.post(G_BASE_URL + '/article/ajax/article_vote/', 'type=article&item_id=' + article_id + '&rating=' + rating, function (result) 
		{
		    ...
		    else
		    {
		    	...
		    	if (rating == 0)
		    	{
		    		selector.parents('.icb-article-vote').find('b')
		    	...
		    	{
		    		if (selector.parents('.icb-article-vote').find('.icon-agree').hasClass('active'))
		    		{
		    			selector.parents('.icb-article-vote').find('b').html(parseInt(selector.parents('.icb-article-vote').find('b').html()) - 1);

		    			selector.find('.icon').removeClass('active');
		    		}
		    		else if (selector.parents('.icb-article-vote').find('.icon-disagree').hasClass('active'))
		    		{
		    			selector.parents('.icb-article-vote').find('b').html(parseInt(selector.parents('.icb-article-vote').find('b').html()) + 1);

		    			selector.parents('.icb-article-vote').find('.icon-disagree').removeClass('active');
		    	...
		    	else
		    	{
		    		if (selector.parents('.icb-article-vote').find('.icon-agree').hasClass('active'))
		    		{
		    			...
		    			selector.parents('.icb-article-vote').find('.icon-agree').removeClass('active');
...
AWS.Dropdown = 
{
	...
	{
		var ul = $(element).parent().find('.icb-dropdown-list ul');
										...
										if (result.length > 0)
										{
											...
											$.each(result, function(i, e)
											{
												switch(result[i].type)
												{
													...
													case 'topics' :
														ul.append('<li class="topic"><span class="article-tag"><a class="text" href="' + decodeURIComponent(result[i].url) + '">' + result[i].name  + '</a></span>&nbsp;<span class="color-999">' + result[i].detail.discuss_count + ' 个讨论</span></li>');
														break;
											...
											ul.show();
											$('.icb-search-result-box .result-mod .all-result').show();
											$('.icb-search-result-box .result-mod .all-result a').attr('href', G_BASE_URL + '/m/search/?q=' + val);
											$('.icb-search-result-box .result-mod .mod-head, .icb-search-result-box .result-mod .tips, .icb-search-result-box .result-mod .icb-load-more').hide();
											$('.icb-search-result-box .btn-primary').show();

											AWS.G.search_val = val;
										}else
										{
											ul.hide();
											$('.icb-search-result-box .result-mod .all-result').hide();
											$('.icb-search-result-box .tips, .icb-search-result-box .btn-primary').show();
							...
							case 'invite' : 
								...
								$.get(G_BASE_URL + '/search/ajax/search/?type=users&q=' + encodeURIComponent($(element).val()) + '&limit=10',function(result)
								{
									if (result.length > 0)
									{
										...
										$('.icb-invite-box ul li a').click(function()
										{
											AWS.User.invite_user($(this),$(this).parents('li').find('img').attr('src'));
										});
										
										$(element).parent().find('.icb-dropdown-list').show();

										ul.show();
									}else
									{
										$(element).parent().find('.icb-dropdown-list').hide();
									}
								},'json');
							break;
							...
							case 'topic' :
								$.get(G_BASE_URL + '/search/ajax/search/?type=topics&q=' + encodeURIComponent($(element).val()) + '&limit=10',function(result)
								{
									if (result.length > 0)
									{
										...
										{
											$(this).parents('.icb-article-title-box').find('.topic-text').val($(this).text());
											$(this).parents('.icb-article-title-box').find('.add').click();
										});
										
										$(element).parent().find('.icb-dropdown-list').removeClass('hide').show();
	...
	set_dropdown_list: function (selecter, data, selected)
	{
	    ...
	    {
	        $('.icb-publish-dropdown span').html($(this).text());
	    });
...
AWS.Init = 
{
	init_topic_edit_box: function (selector, type)
	{
		...
		{
			$(this).parents('.icb-article-title-box').addClass('active');

			var data_id = $(this).parents('.icb-article-title-box').attr('data-id'),
				data_type;
			if (type)
			{
				data_type = type;
			...
			{
				data_type = $(this).parents('.icb-article-title-box').attr('data-type');
			}

			$(selector).hide();

			$(selector).parents('.icb-article-title-box').append(AW_MOBILE_TEMPLATE.topic_edit_box);

			$.each($(selector).parents('.icb-article-title-box').find('.article-tag'), function (i, e)
			{
				if (!$(e).has('i')[0])
				{
			...
			AWS.Dropdown.bind_dropdown_list('.icb-article-title-box input.topic-text','topic');

			/* 话题编辑添加按钮 */
			$('.icb-article-title-box .add').click(function()
			{
				switch (data_type)
				{
					case 'publish' :
						if ($(this).parents('.icb-article-title-box').find('.topic-text').val() != '')
						{
							$(this).parents('.icb-article-title-box').find('.tag-queue-box').prepend('<span class="article-tag"><a class="text">' + $(this).parents('.icb-article-title-box').find('.topic-text').val() + '</a><input type="hidden" name="topics[]" value="' + $(this).parents('.icb-article-title-box').find('.topic-text').val() + '" ><a class="close" onclick="$(this).parents(\'.article-tag\').detach();"><i class="icon icon-delete"></i></a>');
							$(this).parents('.icb-article-title-box').find('.topic-text').val('');
						}
					break;
					case 'question' :
						var _this = $(this);
						$.post(G_BASE_URL + '/topic/ajax/set_article_topic_relation/', 'type=question&item_id=' + data_id + '&topic_title=' + encodeURIComponent($(this).parents('.icb-article-title-box').find('.topic-text').val()), function(result)
						{
							if (result.errno == 1)
							{
								_this.parents('.icb-article-title-box').find('.tag-queue-box').prepend('<span class="article-tag" data-id="'+ result.rsm.topic_id +'"><a class="text">' + _this.parents('.icb-article-title-box').find('.topic-text').val() + '</a><a class="close"><i class="icon icon-delete"></i></a></span>');
								_this.parents('.icb-article-title-box').find('.topic-text').val('');
							}else
							{
								alert(result.err);
							}
					...
					case 'article' :
						var _this = $(this);
						$.post(G_BASE_URL + '/topic/ajax/set_article_topic_relation/', 'type=article&item_id=' + data_id + '&topic_title=' + encodeURIComponent($(this).parents('.icb-article-title-box').find('.topic-text').val()), function (result)
						{
							if (result.errno == 1)
							{
								_this.parents('.icb-article-title-box').find('.tag-queue-box').prepend('<span class="article-tag" data-id="'+ result.rsm.topic_id +'"><a class="text">' + _this.parents('.icb-article-title-box').find('.topic-text').val() + '</a><a class="close"><i class="icon icon-delete"></i></a></span>');
				...
				$(this).parents('.icb-article-title-box').find('.icb-dropdown-list').hide();
			});

			/* 话题编辑取消按钮 */
			$('.icb-article-title-box .cancel').click(function()
			{
				$(this).parents('.icb-article-title-box').find('.icb-add-topic-box').show();
				$.each($(this).parents('.icb-article-title-box').find('.article-tag'), function (i, e)
				{
					if ($(e).has('.close')[0])
					{
						$(e).find('.close').detach();
				...
				$(this).parents('.icb-article-title-box').removeClass('active');
	...
	init_article_comment_box: function (selector)
	{
		$(document).on('click', selector, function ()
	    {
	        if ($(this).parents('.mod-footer').find('.icb-comment-box').length)
	        {
	            if ($(this).parents('.mod-footer').find('.icb-comment-box').css('display') == 'block')
	            {
	               $(this).parents('.mod-footer').find('.icb-comment-box').fadeOut();
	               $(this).removeClass('active');
	            }
	            else
	            {
	                $(this).parents('.mod-footer').find('.icb-comment-box').fadeIn();
	                $(this).addClass('active');
	            }
	        }
	        else
	        {
	            ...
	            $(this).parents('.mod-footer').append(Hogan.compile(AW_MOBILE_TEMPLATE.articleCommentBox).render(
	            {
	                ...
	                'article_id' : $('.icb-replay-box input[name="article_id"]').val()
	            }));
	            $(this).parents('.mod-footer').find('.cancel').click(function ()
	            {
	                $(this).parents('.mod-footer').find('.icb-comment-box').fadeOut();
	            });
	            $(this).parents('.mod-footer').find('.icb-comment-txt').autosize();
